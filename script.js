...
function fetchAppointmentData() {
    return fetch(
            "http://76.166.224.111/FingerprintAppointments/backend.php"
        )
        .then((response) => {
            if (!response.ok) {
...
function createDaySchedule(day, date) {
    ...
    slots.forEach((slot, index) => {
        const appointment = appointmentData.find(a => a.date === dateString && a.time === slot);
...
function saveAppointments() {
    ...
    fetch(
            "http://76.166.224.111/FingerprintAppointments/save_appointments.php", {
                method: "POST",
                headers: {
